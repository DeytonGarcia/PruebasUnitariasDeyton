...
public class WaterBoxAssignmentService implements IWaterBoxAssignmentService {
    ...
    public WaterBoxAssignmentResponse save(WaterBoxAssignmentRequest request) {
        ...
        System.out.println("[WBA SERVICE] Creating assignment for waterBoxId=" + request.getWaterBoxId() + " userId=" + request.getUserId());
        System.out.println("[WBA SERVICE] WaterBox currentAssignmentId=" + waterBox.getCurrentAssignmentId());
        
        // PERMITIR MÚLTIPLES ASIGNACIONES ACTIVAS: NO desactivar asignaciones anteriores
        // Una caja de agua puede estar asignada a múltiples usuarios simultáneamente
        System.out.println("[WBA SERVICE] Allowing multiple active assignments - not deactivating previous assignments");

        // Mapear y asegurar que id == null para forzar INSERT
        WaterBoxAssignment assignment = toEntity(request);
        System.out.println("[WBA SERVICE] Mapped assignment entity, id before clear=" + assignment.getId());
        // defensive: always clear id to force INSERT (in case a client accidentally sent one)
        assignment.setId(null);
        assignment.setStatus(Status.ACTIVE);
        assignment.setCreatedAt(LocalDateTime.now());
        System.out.println("[WBA SERVICE] Assignment prepared for INSERT with id=null");

        // Guardar y forzar el insert inmediato
        WaterBoxAssignment savedAssignment = waterBoxAssignmentRepository.saveAndFlush(assignment);
        System.out.println("[WBA SERVICE] Assignment saved with NEW id=" + savedAssignment.getId());

        // Actualizar la WaterBox con la nueva asignación como "current" (la más reciente)
        waterBox.setCurrentAssignmentId(savedAssignment.getId());
        waterBoxRepository.saveAndFlush(waterBox);
        System.out.println("[WBA SERVICE] WaterBox updated with currentAssignmentId=" + savedAssignment.getId());
    ...
    // Convertir OffsetDateTime (desde DTO) a LocalDateTime (entidad)
    ...
    public WaterBoxAssignmentResponse getActiveAssignmentByUserId(String userId) {
        java.util.List<WaterBoxAssignment> assignments = waterBoxAssignmentRepository
        ...
        // Retorna la asignación más reciente (primera en la lista ordenada por fecha DESC)
